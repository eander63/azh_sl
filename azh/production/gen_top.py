...
def gen_top_decay_products(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    t_children = t.distinctChildrenDeep[
        t.distinctChildrenDeep.hasFlags("isHardProcess")
    ]
    ...
    # distinct W children
    w_children = w.distinctChildrenDeep[
        w.distinctChildrenDeep.hasFlags("isHardProcess")
    ]
...
def top_pt_weight(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    if not self.dataset_inst.has_tag("is_ttbar"):
        raise Exception(
            f"gen_top_pt_weight should only run for ttbar dataset, got {self.dataset_inst}"
        )
    ...
    events = set_ak_column(
        events, "top_pt_weight_down", ak.fill_none(weight * 0.5, 1.0)
    )
