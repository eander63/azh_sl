...
@producer(
    ...
    produces={
        "muon_trig_weight",
        "muon_trig_weight_up",
        "muon_trig_weight_down",
        "electron_trig_weight",
        "electron_trig_weight_up",
        "electron_trig_weight_down",
    },
    mc_only=True,
)
def trigger_weights(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    lead_muo_pt = np.clip(
        lead_muo_pt, 26.1, 199.9
    )  # muon_Z.json HLT SFs valid ~26-200 GeV
    ...
    events = set_ak_column(
        events, "muon_trig_weight", ak.values_astype(muon_trig_weight, np.float32)
    )
    events = set_ak_column(
        events, "muon_trig_weight_up", ak.values_astype(muon_trig_weight_up, np.float32)
    )
    events = set_ak_column(
        events,
        "muon_trig_weight_down",
        ak.values_astype(muon_trig_weight_dn, np.float32),
    )
    ...
    ele_sf = self.electron_trig_corr.evaluate(
        ele_trig_year, "sf", ele_trig_path, lead_ele_eta, lead_ele_pt
    )
    ele_sf_up = self.electron_trig_corr.evaluate(
        ele_trig_year, "sfup", ele_trig_path, lead_ele_eta, lead_ele_pt
    )
    ele_sf_dn = self.electron_trig_corr.evaluate(
        ele_trig_year, "sfdown", ele_trig_path, lead_ele_eta, lead_ele_pt
    )
    ...
    events = set_ak_column(
        events,
        "electron_trig_weight",
        ak.values_astype(electron_trig_weight, np.float32),
    )
    events = set_ak_column(
        events,
        "electron_trig_weight_up",
        ak.values_astype(electron_trig_weight_up, np.float32),
    )
    events = set_ak_column(
        events,
        "electron_trig_weight_down",
        ak.values_astype(electron_trig_weight_dn, np.float32),
    )
