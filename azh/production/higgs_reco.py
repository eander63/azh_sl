...
def higgs_reco(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    h = ak.where((ak.num(events.Jet, axis=-1) == 5), ak.where((ak.num(events.BJet, axis=-1) > 2),
        sorted_jets[:, 0] + sorted_jets[:, 1] + sorted_jets[:, 2] + sorted_jets[:, 3] + sorted_jets[:, 4],
        ak.where((ak.num(events.BJet, axis=-1) == 2),
        sorted_bjets[:, 0] + sorted_bjets[:, 1] + light_jets[:, 0] + light_jets[:, 1] + light_jets[:, 2],
        ak.where((ak.num(events.BJet, axis=-1) == 1),
        ...
        ak.where((ak.num(events.BJet, axis=-1) > 2), sorted_jets[:, 0] + sorted_jets[:, 1] + sorted_jets[:, 2] +
        sorted_jets[:, 3] + sorted_jets[:, 4] + sorted_jets[:, 5],
        ak.where((ak.num(events.BJet, axis=-1) == 2), sorted_bjets[:, 0] + sorted_bjets[:, 1] + light_jets[:, 0] +
        light_jets[:, 1] + light_jets[:, 2] + light_jets[:, 3],
        ak.where((ak.num(events.BJet, axis=-1) == 1), sorted_bjets[:, 0] + light_jets[:, 0] +
        light_jets[:, 1] + light_jets[:, 2] + light_jets[:, 3] + light_jets[:, 4],
        light_jets[:, 0] + light_jets[:, 1] + light_jets[:, 2] + light_jets[:, 3] +
        light_jets[:, 4] + light_jets[:, 5]))))
