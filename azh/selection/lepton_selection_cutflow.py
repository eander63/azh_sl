...
def lepton_selection_cutflow(
    ...
    **kwargs,
    ...
    lep_loose = ((events.cutflow.n_ele_loose > 1) | (events.cutflow.n_muo_loose > 1))
    lep_tight = ((events.cutflow.n_ele > 1) | (events.cutflow.n_muo > 1))
    lep_exact = (((events.cutflow.n_ele == 2) &
    (events.cutflow.n_ele_loose == 2) & (events.cutflow.n_muo_loose == 0)) |
    ((events.cutflow.n_muo == 2)  &
    (events.cutflow.n_muo_loose == 2) & (events.cutflow.n_ele_loose == 0)))

    lep_sel_nosign = (((events.cutflow.n_ele == 2) & (events.cutflow.n_ele_high > 0) &
        (events.cutflow.n_ele_loose == 2) & (events.cutflow.n_muo_loose == 0)) |
    ...
    return events, SelectionResult(
        steps={
            "AtLeast2LooseLept": lep_loose,
            "AtLeast2TightLept": lep_tight,
            "Exactly2TightLept": lep_exact,
            "Exactly2TightLept_OneHighpt": lep_sel_nosign,
            "Exactly2TightLept_OneHighpt_os": lep_sel,
        },
        objects={
            "Electron": {
